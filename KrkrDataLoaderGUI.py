...
def main():
    ...
    def show_large_image():
        ...
        '''
        def get_resized_image(pil_image, target_width, target_height):
            """Resize the image to fit within the target width and height while maintaining aspect ratio."""
        ...
        # 创建新窗口
        new_window = sg.Window('导出效果图预览', [[image]],icon='KrkrDataLoader.ico',background_color='#ffffff',grab_anywhere=True,resizable=True)

        while True:
            event, values = new_window.read(timeout=300)
            if event == sg.WINDOW_CLOSED:
                break
            # 更新图片大小以适应窗口
            new_size = (new_window.size[0] - 10, new_window.size[1] - 10)  # 减少边缘留白
