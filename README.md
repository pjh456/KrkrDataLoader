...
KrkrDataLoader 的再次重构版本，承接了 Python 版本的设计，并修复了大量 bug。

技术上使用 Electron + Java 实现。
...
1. 解析原加密 SCN 剧情文件为 JSON。（目前未加入）
2. 加载 JSON 剧情文件并提供 API 简化这一流程。
3. 提供命令行配置工具实现自动化解析不同类型的剧情文件。
4. 提供 GUI 页面简化剧情解析与浏览过程。
5. 提供对音频文件的支持。（目前未加入）
6. 提供对立绘文件的支持。（目前未加入）
7. 提供可运行程序来播放剧情，实现剧情的重演绎。（目前未加入）  
8. 提供可运行程序来浏览立绘，实现立绘的切换。（目前未加入）
